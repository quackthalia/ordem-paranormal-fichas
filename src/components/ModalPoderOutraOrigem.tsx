...
export const ModalPoderOutraOrigem: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  const renderOrigem = (origem: any) => {
              ...
              return (
                <div key={origem.Codigo_Origem} className={`bg-zinc-900/40 border border-zinc-800/80 rounded p-3 hover:border-green-500/50 hover:bg-zinc-900/80 group flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'min-h-[155px] max-h-[3000px]' : 'min-h-[155px] max-h-[155px]'} cursor-pointer`}>
                  <div 
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="inline-block rounded bg-green-900/30 px-1.5 py-px text-[9px] font-bold uppercase tracking-wider leading-tight text-green-400">{origem.Nome_Poder}</span>
                    </div>
